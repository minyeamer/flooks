...
The repository is still at the skeleton stage, but it now includes a live bootstrap slice, an identity baseline slice, a metadata persistence baseline, a dashboard CRUD slice, and a governed query bootstrap. The web shell reads `/api/v1/system` and `/api/v1/overview` from the FastAPI service, while the API now exposes `/api/v1/identity/bootstrap`, `/api/v1/metadata/bootstrap`, `/api/v1/dashboards`, `/api/v1/query/bootstrap`, and `/api/v1/query/validate` for the first auth, storage, dashboard, and query contracts.

The parts that are already aligned are the monorepo layout, the React shell, the FastAPI skeleton, the identity, metadata, dashboard, and query bootstrap surfaces, the dashboard schema package, and the Compose structure.

The next implementation wave should add the following immediately:

1. connector-backed QuerySpec execution for Linkmerce PostgreSQL marts
2. the panel runtime on top of the new metadata tables
