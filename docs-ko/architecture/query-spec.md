...
현재 backend bootstrap은 governed query surface를 실제 실행 경로까지 포함해 노출한다.

- `GET /api/v1/query/bootstrap` 은 in-memory starter dataset manifest registry를 반환한다.
- `POST /api/v1/query/validate` 는 `QuerySpec` payload를 manifest 계약에 대조해 검증하고, 정규화된 execution preview를 반환한다.
- `POST /api/v1/query/execute` 는 payload를 검증/번역한 뒤 현재 `POSTGRES` connector 경로로 실제 실행 결과를 반환한다.

현재 단계는 계약 검증과 실행을 모두 지원한다. `POSTGRES` 외 connector 확장은 다음 backend 단계다.
...
현재 bootstrap은 semantic validation과 connector-backed 실행을 모두 포함한다. 검증과 실행은 같은 manifest-governed contract를 공유한다.
...
V1은 `POSTGRES` connector 경로를 먼저 지원하고, 이후 BigQuery, ClickHouse, JDBC, CSV connector가 추가될 수 있다. QuerySpec은 이때 frontend와 AI의 호출 방식을 바꾸지 않고 backend translator만 교체할 수 있게 한다.
