...
현재 backend bootstrap은 첫 governed query surface를 이미 노출한다.
...
이 단계는 계약 검증까지만 수행한다. Linkmerce PostgreSQL mart를 대상으로 하는 connector-backed execution은 다음 backend 단계다.
...
현재 bootstrap은 semantic validation과 execution preview 생성까지만 구현돼 있고, connector SQL 실행은 아직 포함하지 않는다.
